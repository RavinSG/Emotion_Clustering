#
# Want inmemory or disk cache? We build them here
#
...
from config import GLOVE_PATH, DOC2VEC_PATH
from config import GOOGLE_E2V_PATH
from config import GOOGLE_W2V_PATH
from config import TEMP_PATH
from config import TWITTER_W2V_PATH
from config import WIKIS_FT_PATH
from config import WIKI_FT_PATH
...
def twitter_word2vec(verbose=False):
    ...
    word_vec = load_word2vec(
        TWITTER_W2V_PATH, binary=True, unicode_errors='ignore')
    end = time.time()
    if verbose:
        print('\t[{} seconds]'.format(end - start))
...
def wiki_fasttext(subword=False, verbose=False):
    ...
    start = time.time()
    if subword:
        word_vec = load_embedding(
            WIKIS_FT_PATH, word_first=True, leave_head=True)
    else:
        word_vec = load_embedding(
            WIKI_FT_PATH, word_first=True, leave_head=True)
    end = time.time()
    if verbose:
        print('\t[{} seconds]'.format(end - start))
...
def context2vec(tweets, verbose=False):
    ...
    if context2vec_cache is None:
        context2vec_cache = DiskCache(
            os.path.join(TEMP_PATH, 'context2vec.cache'))
